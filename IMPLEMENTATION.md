...
| `generate_script.py` | **Not used for long-form scripting** — a near-unmodified Shorts copy, still Shorts-shaped in its docstring/prompt/length targets. Kept only because `run_pipeline.py`'s `--start-from script` guard needs something to exist to block; scripts are written by hand against `Workflows/pipeline_script_prompt_template.md` instead. |
